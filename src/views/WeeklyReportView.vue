  ...
  methods: {
    ...
    async loadReportData() {
      ...
      try {
        ...
        // 残高データ読み込み（取引データと同じ範囲）
        const balanceResponse = await ApiService.getDailyBalances(null, apiStart, apiEnd)
        console.log('残高APIレスポンス:', balanceResponse)
        console.log('残高データ一覧:', balanceResponse.balances)
    ...
    getDailyBalance(accountType, date) {
      ...
      const searchKey = accountType === 'cash' ? 'cash' : `bank-${accountType}`
      const balance = this.balances.find(b => b.date === date && b.accountId === searchKey)
